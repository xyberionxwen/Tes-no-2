...
mata pelajaran. Sistem penilaian telah disesuaikan dengan KBK sehingga masingmasing
...
def search_word(article, word):
    """Menghitung berapa kali kata ditemukan dalam artikel."""
    return article.lower().split().count(word.lower())

def replace_word(article, old_word, new_word):
    """Mengganti semua kemunculan kata tertentu dengan kata baru."""
    return article.replace(old_word, new_word)

def sort_words(article):
    """Mengurutkan kata-kata dalam artikel secara abjad."""
    words = article.lower().split()
    words = [word.strip('.,') for word in words]  # Menghilangkan tanda baca
    return sorted(set(words))  # Menggunakan set untuk menghindari duplikasi

def menu_program():
    global article #Declare article as global outside the function
    while True:
        try:
            print("\n=== Program Artikel ===")
            ...
            if choice == "1":
                ...
                article = replace_word(article, old_word, new_word) # removed global
